	...
	float3 rdims)			// # of regions in (x,y,z)
{
	...
	{	
		...
		if(coord_in_volume.x <= (roi_offset.x + roi_dim.x) || 
			...
			dc_dv_row = &dc_dv[3 * vox_per_rgn.x * vox_per_rgn.y * vox_per_rgn.z * pidx];
			...
			if ((displacement_in_vox.x < -0.5) || (displacement_in_vox.x > (volume_dim.x - 0.5)) || 
				...
				dc_dv_row[3*threadIdxInGrid+0] = diff * tex1Dfetch(tex_moving_grad, (3 * (int)mvr) + 0);
				dc_dv_row[3*threadIdxInGrid+1] = diff * tex1Dfetch(tex_moving_grad, (3 * (int)mvr) + 1);
				dc_dv_row[3*threadIdxInGrid+2] = diff * tex1Dfetch(tex_moving_grad, (3 * (int)mvr) + 2);
			}		
		}
	}
}
