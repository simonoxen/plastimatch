...
parse_args (int argc, char* argv[])
{
	...
	true,"","string",cmd);
    TCLAP::ValueArg<std::string> arg_output_type (
	"","output-type","Output type {uchar,short,ushort,long,float}",
	false,"","string",cmd);
    TCLAP::ValueArg<std::string> arg_pattern (
	"","pattern","Output pattern {gauss,rect,sphere}",
	false,"","string",cmd);

    TCLAP::ValueArg<std::string> arg_origin (
	"","origin",
	"Origin of output volume \"x [y z]\" (float)",
	false,"","string",cmd);
    TCLAP::ValueArg<std::string> arg_dim (
	"","dim",
	"Number of voxels in output volume \"x [y z]\" (int)",
	false,"","string",cmd);
    TCLAP::ValueArg<std::string> arg_spacing (
	"","spacing",
	"Spacing between voxels in output volume \"x [y z]\" (float)",
	false,"","string",cmd);
    TCLAP::ValueArg<std::string> arg_volume_size (
	"","volume-size",
	"Total size of output volume in mm \"x [y z]\" (float)",
	false,"","string",cmd);

    TCLAP::ValueArg<std::string> arg_background (
	"","background",
	"Intensity of pattern background (float)",
	false,"","string",cmd);
    TCLAP::ValueArg<std::string> arg_foreground (
	"","foreground",
	"Intensity of pattern foreground (float)",
	false,"","string",cmd);

    TCLAP::ValueArg<std::string> arg_gauss_center (
	"","gauss-center",
	"Location of Gaussian center \"x [y z]\" (float)",
	false,"","string",cmd);
    TCLAP::ValueArg<std::string> arg_gauss_std (
	"","gauss-std",
	"Standard deviation of Gaussian pattern (in mm) \"x [y z]\" (float)",
	false,"","string",cmd);

    TCLAP::ValueArg<std::string> arg_rect_size (
	"","rect-size",
	"Size of rectangle pattern (in mm).  Specify as \"x [y z]\" "
	"or \"x1 x2 y1 y2 z1 z2\" (float)",
	false,"","string",cmd);

    TCLAP::ValueArg<std::string> arg_sphere_center (
	"","sphere-center",
	"Location of sphere center \"x [y z]\" (float)",
	false,"","string",cmd);
    TCLAP::ValueArg<std::string> arg_sphere_radius (
	"","sphere-radius",
	"Radius of sphere pattern (in mm) (float)",
	false,"","string",cmd);
...
#if defined (commentout)
    /* Copy arguments to parms structure */
    if (arg_output.isSet()) {
