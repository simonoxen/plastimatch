...
)
{
     ...
     *
     *   -- anything with the g_ prefix defines the scattering
     *      region surrounding the ct_xyz voxel.  (i.e. The
     *      Gaussian kernel region of influence).
     *
     *   -- scatter[3] contains the 3D real space coordinates
     *      of the current voxel under consideration within
     *      the scattering region defined within g_
     *
...
//    sigma = 2.0f;   // << kludge
    ...
    /* Scan the scatter radius starting from the top left */
    vec3_copy (g_start, ct_xyz);
    vec3_scale3 (tmp, g_incr_x, - search_dist);
    vec3_add2 (g_start, tmp);
    vec3_scale3 (tmp, g_incr_y, - search_dist);
    vec3_add2 (g_start, tmp);

    limit = 2*((search_dist + g_step - 1) / g_step);
    
#if defined (commentout)
    sane = 0;
#endif
    /* Step along y-dim parallel to aperture plane */
    for (g_y = 0; g_y < limit; g_y++) {
        double g_pos[3];
        double tmp[3];
        ...
        for (g_x = 0; g_x < limit; g_x++) {
            ...
            depth_rg = get_rgdepth (scatter, depth_offset,
                             depth_vol, pep,
                             pmat, ires, ic_room,
                             incr_r, incr_c,
                             options);
            ...
            if (depth_rg < 0.0f) {
                /* Neighbor not hit directly by beam, so no scattering */
                continue;
            } else {
                /* Lookup the dose at neighbor's radiographic depth */
