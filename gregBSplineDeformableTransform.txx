...
=========================================================================*/
#ifndef _gregBSplineDeformableTransform_txx
#define _gregBSplineDeformableTransform_txx
...
gregBSplineDeformableTransform<TScalarType, NDimensions,VSplineOrder>
::gregBSplineDeformableTransform():Superclass(SpaceDimension,0)
{

  // Instantiate a weights function
  m_WeightsFunction = WeightsFunctionType::New();
#if defined (commentout)
  printf ("Got m_WeightsFunction: num_weights = %d\n", m_WeightsFunction->GetNumberOfWeights());
#endif
...
template<class TScalarType, unsigned int NDimensions, unsigned int VSplineOrder>
gregBSplineDeformableTransform<TScalarType, NDimensions,VSplineOrder>
::~gregBSplineDeformableTransform()
{
...
::SetParameters( const ParametersType & parameters )
{
    ...
    printf ("SetParameters called\n");
    std::cout << "SIZE:   " << parameters.Size() << std::endl;
    std::cout << "NPARAM: " << this->GetNumberOfParameters() << std::endl;
...
template<class TScalarType, unsigned int NDimensions, unsigned int VSplineOrder>
const 
typename gregBSplineDeformableTransform<TScalarType, NDimensions,VSplineOrder>
::ParametersType &
gregBSplineDeformableTransform<TScalarType, NDimensions,VSplineOrder>
...
{
    bool inside = true;
    typename ContinuousIndexType::ValueType tmp_index;
	...
	/* GCS */
	for ( unsigned int j = 0; j < SpaceDimension; j++ ) {
	    tmp_index = static_cast<typename IndexType::IndexValueType>(
		    BSplineFloor( index[j] - static_cast<double>( SplineOrder / 2 ) ) );
#if defined (commentout)
#endif
	    if (tmp_index >= m_ValidRegionLast[j]) {
		inside = false;
		break;
	    }
	}
  ...
  bool& inside ) const
{
	...
	debug_printf ("TRANSFORM point: %g %g %g\n", point[0], point[1], point[2]);
	debug_printf ("TRANSFORM m_GridOrigin: %g %g %g\n", m_GridOrigin[0], m_GridOrigin[1], m_GridOrigin[2]);
	debug_printf ("TRANSFORM m_GridSpacing: %g %g %g\n", m_GridSpacing[0], m_GridSpacing[1], m_GridSpacing[2]);
	for ( j = 0; j < SpaceDimension; j++ )
	{
	    index[j] = ( point[j] - m_GridOrigin[j] ) / m_GridSpacing[j];
	}
	debug_printf ("TRANSFORM index: %g %g %g\n", index[0], index[1], index[2]);
	...
	if ( !inside )
	{
	    debug_printf ("TRANSFORM <<not inside>>\n");
	...
	debug_printf ("TRANSFORM supportIndex: %d %d %d\n", supportIndex[0], supportIndex[1], supportIndex[2]);
	if (supportIndex[1] == 24) {
	    m_WeightsFunction->Evaluate( index, weights, supportIndex );
	    inside = this->InsideValidRegion( index );
	}
	...
	while ( ! m_Iterator[0].IsAtEnd() )
	{
	    ...
	    debug_printf ("Counter = %d\n"
			    "m_Iterator[0].m_Index = %d %d %d\n",
			    counter,
			    m_Iterator[0].GetIndex()[0],
			    m_Iterator[0].GetIndex()[1],
			    m_Iterator[0].GetIndex()[2]
			    );
#endif
	    // multiply weigth with coefficient
	    for ( j = 0; j < SpaceDimension; j++ )
	    {
		outputPoint[j] += static_cast<ScalarType>(weights[counter] * m_Iterator[j].Get());
	    }
...
::TransformPoint(const InputPointType &point) const 
{
  ...
  OutputPointType outputPoint;
  bool inside;
  debug_printf ("Gonna transform! #weights = %d\n", m_WeightsFunction->GetNumberOfWeights());
