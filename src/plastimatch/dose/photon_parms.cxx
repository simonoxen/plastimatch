...
public:
    Photon_parms_private () {
        ...
        this->plan = Photon_plan::New ();
    }
};

Photon_parms::Photon_parms ()
{
    this->d_ptr = new Photon_parms_private;
...
void
Photon_parms::handle_end_of_section (int section)
{
    switch (section) {
    case 0:
        /* Settings */
...
Photon_plan::Pointer& 
Photon_parms::get_plan ()
{
    return d_ptr->plan;
}

void
Photon_parms::parse_config (
...
bool
Photon_parms::parse_args (int argc, char** argv)
{
    int i;
    for (i=1; i<argc; i++) {
        if (argv[i][0] != '-') break;
