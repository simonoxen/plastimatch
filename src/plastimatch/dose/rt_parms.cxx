...
Rt_parms::print_verif(Rt_plan::Pointer plan)
{
    ...
    printf("\n beam_weight: "); for (int i = 0; i <= d_ptr->beam_number; i++) {printf("%g ** ", plan->beam_storage[i]->get_beamWeight());}
...
)
{
...
#if (CUDA_FOUND)
                d_ptr->threading = THREADING_CUDA;
#elif (OPENMP_FOUND)
                d_ptr->threading = THREADING_CPU_OPENMP;
#else
                d_ptr->threading = THREADING_CPU_SINGLE;
    ...
    case 2:
        if (!strcmp (key, "flavor")) {
            ...
            if (sscanf (val, "%f", &(d_ptr->beam_weight)) != 1) {
                goto error_exit;
            }
        }
            ...
            if (sscanf (val, "%lf", &d_ptr->photon_energy) != 1) {
                goto error_exit;
            }
        }
        else {
