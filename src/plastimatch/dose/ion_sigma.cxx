...
void compute_sigmas(Ion_plan* ion_plan, float energy, float* sigma_max, std::string size) //Rpl_volume* sigma_vol, Rpl_volume* ct_vol, float energy, float spacing_z, float* sigma_max)
{
    ...
    else if (size == "large")
    {
        ...
        ct_vol = ion_plan->ct_vol_density_lg;
        rgl_vol = ion_plan->rpl_vol_lg;
    }
    else
    {
    ...
    {            
        compute_sigma_source(sigma_vol, rgl_vol, ion_plan, energy);
    }
    else
    {
...
float compute_sigma_pt_homo(Rpl_volume* sigma_vol, Rpl_volume* rpl_vol, float energy)
{
    ...
    for (int i = 0; i < dim; i++)
    {
        if (!rpl_vol->get_aperture()->have_aperture_image() || (rpl_vol->get_aperture()->have_aperture_image() && ap_img[i] > 0))
        {
            ...
            {
                x_over_range = sigma_volume[i] / range;
...
float compute_sigma_pt_hetero(Rpl_volume* sigma_vol, Rpl_volume* rgl_vol, Rpl_volume* ct_vol, float energy)
{
    ...
    float* range_length_img = (float*) rgl_vol->get_vol()->img;
...
void compute_sigma_source(Rpl_volume* sigma_vol, Rpl_volume* rpl_volume, Ion_plan* ion_plan, float energy)
{
    ...
    unsigned char* ap_img = (unsigned char*) ion_plan->get_aperture()->get_aperture_volume()->img;
    ...
    for (int i = 0; i < dim[0] * dim[1]; i++)
    {
        if (ap_img[i] > 0)
        {
            ...
            dist_cp = vec3_dist(ray_data->cp,ion_plan->beam->get_source_position());
            ...
            for (int j = 0; j < dim[2]; j++)
            {
                ...
                {
                    idx = dim[0]*dim[1]*j + i;
                    sigma = ion_plan->beam->get_source_size() * ((dist_cp + proj * (float) j * sigma_vol->get_vol()->spacing[2])/ ion_plan->get_aperture()->get_distance() - 1);
...
void compute_sigma_range_compensator(Rpl_volume* sigma_vol, Rpl_volume* rpl_volume, Ion_plan* ion_plan, float energy)
{
    ...
    float* sigma_img = (float*) sigma_vol->get_vol()->img;
    float* rgl_img = (float*) rpl_volume->get_vol()->img;
    float* rc_img = (float*) ion_plan->get_aperture()->get_range_compensator_volume ()->img;

    unsigned char* ap_img;
    if (rpl_volume->get_aperture()->have_aperture_image())
    {
        ap_img = (unsigned char*) ion_plan->get_aperture()->get_aperture_volume()->img;
    }
