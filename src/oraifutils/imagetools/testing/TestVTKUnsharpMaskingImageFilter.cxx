...
int main(int argc, char *argv[])
{
  ...
  VERBOSE(<< "\nTesting VTKUnsharpMaskingImageFilter.\n")
  ...
  vtkSmartPointer<ora::VTKUnsharpMaskingImageFilter> f = vtkSmartPointer<ora::VTKUnsharpMaskingImageFilter>::New();
  ...
  int ir = -1;
  for (int r = 0; r <= 25; r++) // r=0 --> auto-radius
  {
    if (r == 0)
      f->SetAutoRadius(true);
    else
      f->SetAutoRadius(false);
    refImage = NULL;
    for (int th = 1; th <= 4; th++)
    {
      ...
      if (th == 1)
      {
        ...
        if (r == 0)
          ir = f->GetInternalRadius(); // reference
        else
          ir = -1;
      }
      else
      ...
      VERBOSE(<< "\n " << spre << " radius = " << ((r > 0) ? r : ir) << " , #threads = " << th << " (check = " << s << "): " << (tl->GetElapsedTime() / 10. * 1000.) << " ms" << spost << "\n")

      if (ImageOutput)
      {
        char buff[200];
        sprintf(buff, "2dimage_unsharp_masking_r%d_th%d.mhd", r, th);
  ...
  f = vtkSmartPointer<ora::VTKUnsharpMaskingImageFilter>::New();
  ...
  for (int r = 0; r <= 25; r++) // r=0 --> auto-radius
  {
    ...
    for (int th = 1; th <= 4; th++)
    {
      ...
      {
        char buff[200];
        sprintf(buff, "2dimage_float_unsharp_masking_r%d_th%d.mhd", r, th);
        w->SetInput(f->GetOutput());
        w->SetCompression(false);
        w->SetFileName(buff);
        w->Write();
  ...
  lok = true;
  VERBOSE(<< " -- FIXME (wait for mean filter 3D implementation) -- ") // FIXME
  ok = ok && lok; // update OK-flag for test-scope
  VERBOSE(<< (lok ? "OK" : "FAILURE") << "\n")
