...
/*=========================================================================
  ...
  Module:    $RCSfile: vtkMetaImageWriter2.cxx,v $
...
//----------------------------------------------------------------------------
vtkCxxRevisionMacro(vtkMetaImageWriter2, "$Revision: 1.17 $");
vtkStandardNewMacro(vtkMetaImageWriter2);

//----------------------------------------------------------------------------
vtkMetaImageWriter2::vtkMetaImageWriter2()
  : vtkImageWriter()
{
  this->MHDFileName = 0;
...
//----------------------------------------------------------------------------
void vtkMetaImageWriter2::SetFileName(const char* fname)
{
...
//----------------------------------------------------------------------------
void vtkMetaImageWriter2::SetRAWFileName(const char* fname)
{
  this->Superclass::SetFileName(fname);
}

//----------------------------------------------------------------------------
char* vtkMetaImageWriter2::GetRAWFileName()
{
  return this->Superclass::GetFileName();
}

//----------------------------------------------------------------------------
void vtkMetaImageWriter2::Write( )
{
...
//----------------------------------------------------------------------------
void vtkMetaImageWriter2::PrintSelf(ostream& os, vtkIndent indent)
{
...
void
vtkMetaImageWriter2
::SetInputAndInitializeMetaImage(vtkImageData *input)
{
  this->SetInput(input);
  ...
  if (input)
  {
    ...
    */
    ...
    // NOTE: after calling this method the metaimage can be extended, e.g.
    // set a transformation matrix. BUT be careful!
  }
}
...
::WriteMetaImage()
{
  ...
  if (this->GetInput() == NULL )
    {
    vtkErrorMacro(<<"No input data to write!");
    return;
    }

  if ( !this->MHDFileName )
    {
    vtkErrorMacro("No output file name not specified");
    return;
    }

  // NOTE: we assume that the conventional and possibly further metaimage-
