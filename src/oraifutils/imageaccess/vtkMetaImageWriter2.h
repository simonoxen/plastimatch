...
// This class was modified to make the internal metaimage-object available and
// to allow some manipulations on it before writing it.
// NOTE: the method SetInputAndInitializeMetaImage() must be used instead of
// SetInput(), and WriteMetaImage() instead of Write() to show effect on
// metaimage-object changes.
...
=========================================================================*/
// .NAME vtkMetaImageWriter2 - write a binary UNC meta image data
// .SECTION Description
...
#ifndef __vtkMetaImageWriter2_h
#define __vtkMetaImageWriter2_h
...
class vtkMetaImageWriter2 : public vtkImageWriter
{
public:
  vtkTypeRevisionMacro(vtkMetaImageWriter2,vtkImageWriter);
  void PrintSelf(ostream& os, vtkIndent indent);

  // Description:
  // Construct object with FlipNormals turned off and Normals set to true.
  static vtkMetaImageWriter2 *New();
  ...
  /**
   * Get the pointer to the internal metaimage-object.
   */
  virtual vtkmetaio::MetaImage *GetMetaImagePtr()
  {
    return MetaImagePtr;
  }

  /**
   * Method for setting the input and initializing the metaimage-object if the
   * input is valid.
   * Manipulations on the metaimage-object later, therefore, show an effect.
   * @param input the image data to be written
   */
  virtual void SetInputAndInitializeMetaImage(vtkImageData *input);

  /**
   * Write the meta image which has possibly some extended attributes.
   */
  void WriteMetaImage();

protected:
  vtkMetaImageWriter2();
  virtual ~vtkMetaImageWriter2();
...
private:
  vtkMetaImageWriter2(const vtkMetaImageWriter2&);  // Not implemented.
  void operator=(const vtkMetaImageWriter2&);  // Not implemented.
