...
void
PowellOptimizerEx
::SetLine(const PowellOptimizerEx::ParametersType & origin,
...
{
  PowellOptimizerEx::ParametersType tempCoord( m_SpaceDimension );
                        ...
                        ParametersType & tempCoord)
{
  ...
  for (m_CurrentLineIteration = 0;
    ...
    if (m_Stop) // phil: support stopping!
      return;
    ...
    if( functionValueOft <= functionValueOfX )
      ...
      // phil: support iteration events of internal optimization
...
::StartOptimization()
{
  ...
  PowellOptimizerEx::ParametersType p(m_SpaceDimension);
  PowellOptimizerEx::ParametersType pt(m_SpaceDimension);
  PowellOptimizerEx::ParametersType ptt(m_SpaceDimension);
  ...
  for (m_CurrentIteration = 0;
    ...
    for (unsigned int i = 0; i < m_SpaceDimension; i++)
      ...
      if (m_Stop) // phil: support stopping
      {
        m_StopConditionDescription << "Manual stop requested!";
