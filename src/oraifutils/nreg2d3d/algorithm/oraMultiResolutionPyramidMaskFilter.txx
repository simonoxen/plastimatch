#ifndef ORAMULTIRESOLUTIONPYRAMIDMASKFILTER_TXX
#define ORAMULTIRESOLUTIONPYRAMIDMASKFILTER_TXX
...
MultiResolutionPyramidMaskFilter<TMaskImage>
...
::GenerateData()
{
  ...
  typedef itk::ImageToImageFilter<TMaskImage,TMaskImage> ImageToImageType;
  typedef itk::ResampleImageFilter<TMaskImage,TMaskImage> ResampleShrinkerType;
  typedef itk::ShrinkImageFilter<TMaskImage,TMaskImage> ShrinkerType;
  ...
  // Setup the filters
  shrinkerFilter->SetInput( inputPtr );
...
::GenerateOutputRequestedRegion(DataObject * refOutput )
{
  ...
  TMaskImage * ptr = static_cast<TMaskImage*>( refOutput );
  if( !ptr )
    {
  ...
  if ( ptr->GetRequestedRegion() == ptr->GetLargestPossibleRegion() )
    ...
    for( idim = 0; idim < TMaskImage::ImageDimension; idim++ )
      {
...
#endif  // ORAMULTIRESOLUTIONPYRAMIDMASKFILTER_TXX
