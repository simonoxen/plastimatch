...
void PrintUsage(char *binname)
{
  ...
  std::cout
      << "  -ec or --evolutionary-config ... evolutionary optimizer configuration: max. iterations, initial radius, growth factor, shrink factor, epsilon (min Frobenius norm) [default: 200 1.01 1.05 0.98 0.1] \n";
  std::cout
      << "  -es or --evolutionary-seed ... seed for deterministic behavior [optional]\n";
  std::cout
...
void OptimizerEvent(itk::Object *obj, const itk::EventObject &ev, void *cd)
{
  if (std::string(ev.GetEventName()) == "IterationEvent")
  {
    ...
    if (GraphWin && MetricTable)
    {
      ...
      MetricIterationsVec.push_back(opt->GetCurrentIteration());
      MetricValuesVec.push_back(opt->GetValue());
      ...
      std::string resstring = MakeParametersHumanReadable(
          opt->GetCurrentPosition(), true);
    ...
    {
      // -> only if value becomes better:
      double curr = opt->GetValue();
      if (curr < BestValue)
      {
        doUpdate = true;
        BestValue = curr; // store
      }
    }
...
int main(int argc, char *argv[])
{
  ...
  for (std::size_t i = 0; i < fixedImages.size(); i++)
  {
    ...
    if (metricConfigs[i]->id == "SRCMetricConfig") // orig: -> MIN
    {
      ...
      //    SRCMetricType::ScalesType mdscales;
    ...
    else if (metricConfigs[i]->id == "NMIMetricConfig") // orig: -> MAX
    {
      ...
      // not used at the moment:
      //    NMIMetricType::ScalesType mdscales;
      //    mdscales.SetSize(transform->GetNumberOfParameters());
      //    for (unsigned int d = 0; d < transform->GetNumberOfParameters(); d++)
      //      mdscales[d] = dscale[d];
      //    m->SetDerivativeScales(mdscales);
...
//    MetricTable->SetNumberOfRows(maxIter + 10); // approximately
//    for (vtkIdType i = 0; i < MetricTable->GetNumberOfRows(); i++)
//    {
//      MetricTable->SetValue(i, 0, i + 1);
//      MetricTable->SetValue(i, 1, 0);
//    }
