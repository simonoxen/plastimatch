...
void PrintUsage(char *binname)
{
  ...
  std::cout << "\n";
  std::cout << "   *** N R E G 2 D 3 D _ 3   U S A G E ***\n";
  std::cout << "\n";
  std::cout << progname
  ...
  std::cout
      << "  -sc or --spsa-config ... SPSA optimizer configuration: max. iterations, a, A, alpha, c, gamma, pertubations, initial step size (if >0 -> auto) [default: 200 1.0 20 0.6 1.0 0.1 1 0] \n";
  std::cout
...
/** if >0, SPSA is automatically adjusted (a and A) **/
double SPSAInitialAutoStepSize = 0;
...
void OptimizerEvent(itk::Object *obj, const itk::EventObject &ev, void *cd)
{
  if (std::string(ev.GetEventName()) == "IterationEvent")
  {
    ...
    double lr = opt->GetLearningRate();
    double gm = opt->GetGradientMagnitude();
    double soc = opt->GetStateOfConvergence();
...
/** Multi-resolution level observer. **/
void MRLevelEvent(itk::Object *obj, const itk::EventObject &ev, void *cd)
{
  if (SPSAInitialAutoStepSize > 0)
  {
    RegistrationType *nreg = (RegistrationType *)cd;
    OptimizerType *opt = (OptimizerType *)nreg->GetOptimizer();
    opt->GuessParameters(1, SPSAInitialAutoStepSize);
    std::cout << "      Auto-Setup: " << SPSAInitialAutoStepSize << "\n";
    std::cout << "      auto -> a: " << opt->Geta() << "\n";
    std::cout << "      auto -> A: " << opt->GetA() << "\n";
    std::cout << "      (grad: " << opt->GetGradient() << ")\n";
  }
}
...
int main(int argc, char *argv[])
{
  ...
  for (int i = 1; i < argc; i++)
  {
        ...
        == "--spsa-config")
    {
      ...
      sa_c = atof(argv[i]);
      i++;
      sa_gamma = atof(argv[i]);
      i++;
      sa_pertub = atoi(argv[i]);
      i++;
      SPSAInitialAutoStepSize = atof(argv[i]);
      i++;
    }
  }
  if ((last + 3) > argc)
  ...
  opt->Seta(sa_a);
  opt->SetA(sa_A);
  opt->SetAlpha(sa_alpha);
  opt->Setc(sa_c);
  opt->SetGamma(sa_gamma);
  opt->SetMaximumNumberOfIterations(maxIter);
  opt->SetNumberOfPerturbations(sa_pertub);
  ...
  CommandType::Pointer mrcmd = CommandType::New();
  mrcmd->SetClientData(nreg);
  mrcmd->SetCallback(MRLevelEvent);
  nreg->AddObserver(ora::StartMultiResolutionLevelEvent(), mrcmd);
