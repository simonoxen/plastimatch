...
void Acquire_4030e_parent::timer_event () //will be runned from the first time.
{
    ...
    if (gen_panel_select == 0
        && m_enPanelStatus[0] == STANDBY_SIGNAL_DETECTED
        && m_enPanelStatus[1] == STANDBY_SIGNAL_DETECTED)
    {
    ...
    // if panel is not ready but only gen state and expose request
    // should not trigger the relay.
    // at least do nothing with panel when it is not ready
