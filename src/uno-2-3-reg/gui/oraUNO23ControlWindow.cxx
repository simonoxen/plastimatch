/*
 TRANSLATOR ora::UNO23ControlWindow
...
#include "oraUNO23ControlWindow.h"
...
UNO23ControlWindow::UNO23ControlWindow(QWidget *parent) :
...
UNO23ControlWindow::~UNO23ControlWindow()
{
  if (m_MainTimer)
    delete m_MainTimer;
  m_MainTimer = NULL;
...
void UNO23ControlWindow::Initialize()
{
  m_Initializing = true;
...
//  m_StatusWidget = new UNO23TaskPresentationWidget(this);
...
//  statusBar()->setVisible(true);
  ...
  m_StatusWidget->SetCancelButtonToolTip(UNO23ControlWindow::tr(
      "Stop current running process."));
  m_StatusWidget->SetStartButtonToolTip(UNO23ControlWindow::tr(
  ...
  xyp->SetXAxisLabel(UNO23ControlWindow::tr("Iterations").toStdString());
  xyp->SetXTicksNumberFormat('i');
  xyp->SetYAxisLabel(UNO23ControlWindow::tr("Cost Function").toStdString());
  xyp->SetYTicksNumberFormat('f');
  xyp->SetXTicksNumberPrecision(3);
  xyp->ClearCurve(0);
  xyp->SetCustomText(XYPlotWidget::CTT_CSV_SAVE_DLG_DEF_FILE,
      UNO23ControlWindow::tr("curves.csv"));
  xyp->SetCustomText(XYPlotWidget::CTT_CSV_SAVE_DLG_FILE_FILTER,
      UNO23ControlWindow::tr("CSV sheet (*.csv)"));
  xyp->SetCustomText(XYPlotWidget::CTT_CSV_SAVE_DLG_TITLE,
      UNO23ControlWindow::tr("Export curves to CSV sheet ..."));
  xyp->SetCustomText(XYPlotWidget::CTT_CSV_TT,
      UNO23ControlWindow::tr("Export all curves to a single CSV sheet."));
  xyp->SetCustomText(XYPlotWidget::CTT_VIS_CMB_TT,
      UNO23ControlWindow::tr("Specify which curves should be displayed."));
  xyp->SetCustomText(XYPlotWidget::CTT_ZOOM_FULL_TT,
      UNO23ControlWindow::tr("Fit display to extent of all visible curves."));
  xyp->SetCustomText(XYPlotWidget::CTT_ZOOM_IN_TT,
      UNO23ControlWindow::tr("Zoom in."));
  xyp->SetCustomText(XYPlotWidget::CTT_ZOOM_OUT_TT,
      UNO23ControlWindow::tr("Zoom out."));
  ...
  if (m_CastedModel->IsScientificMode())
    this->setWindowTitle(m_WindowTitle + " " +
        UNO23ControlWindow::tr("[Scientific Mode]"));

  m_Initializing = false;
  m_Initialized = true;
}

void UNO23ControlWindow::Update(int id)
{
  if (id == UNO23Model::UPDATE_COST_FUNCTION)
  {
    m_UpdateCostFunctionFlag = true;
  }
  else if (id == UNO23Model::UPDATE_PARAMETERS)
  {
...
void UNO23ControlWindow::SetLoadConfigVisible(bool visible)
{
  ui.LoadConfigButton->setVisible(visible);
}

void UNO23ControlWindow::OnMainTimerTimeout()
{
  if (m_Initialized)
  {
    // initial configuration loading (not a task):
    m_StatusWidget->ShowMessage(UNO23ControlWindow::tr("Loading configuration ..."));
    ...
    if (!m_CastedModel->HaveValidConfiguration())
    {
      ...
      QMessageBox::critical(this, UNO23ControlWindow::tr("Configuration Error"),
          UNO23ControlWindow::tr("The specified configuration file appeared to be invalid!\nError occurred here: [%1]\\%2\nError description: %3").
      ...
      m_StatusWidget->ShowMessage(UNO23ControlWindow::tr("Configuration error detected."));
      return;
    }
    m_StoredCurveVisibility.clear();
    ...
    {
      QString message = UNO23ControlWindow::tr("The hardware (graphics card, GPU) appears to be invalid for UNO-2-3-REG. The application cannot resume!\nPossible reasons:\n");
      message += QString::fromStdString(nonSupportReasons);
      QMessageBox::critical(this, UNO23ControlWindow::tr("Hardware Error"), message);
    ...
    if (m_CastedModel->HaveValidImageFileNames())
    {
      ...
      vtask->OverrideName(UNO23ControlWindow::tr("Importing volume ..."));
      ...
      if (vtask && ftasks.size() > 0)
      {
        ...
        {
          last = ftasks[i];
          ftasks[i]->OverrideName(UNO23ControlWindow::tr(
      ...
      else
      {
        if (vtask)
          delete vtask;
        QMessageBox::critical(this, UNO23ControlWindow::tr("Image import"),
            UNO23ControlWindow::tr("Image import could not be established."));
      }
    }
    else
    {
      QMessageBox::critical(this, UNO23ControlWindow::tr("Images"),
          UNO23ControlWindow::tr("No or invalid images configured."));
    }
  }
...
void UNO23ControlWindow::OnLastImageLoaderTaskFinished(bool execute)
{
  ...
  if (task && m_CastedModel->GetTaskManager())
  {
    ...
    task->SetNames(
        UNO23ControlWindow::tr("Pre-processing images ..."),
        UNO23ControlWindow::tr("Generating mask images ..."),
        UNO23ControlWindow::tr("Initializing framework ..."),
        UNO23ControlWindow::tr("Post-processing images ..."));
  ...
  else
  {
    QMessageBox::critical(
        this,
        UNO23ControlWindow::tr("Registration initialization"),
...
void UNO23ControlWindow::OnOKButtonPressed()
{
  bool ok = true;
  ...
  if (!ok)
  {
    QMessageBox::critical(
      this,
      UNO23ControlWindow::tr("Registration result storage"),
      UNO23ControlWindow::tr(
...
void UNO23ControlWindow::OnCancelButtonPressed()
{
  this->close(); // rest of code is implemented in closeEvent()!
}

void UNO23ControlWindow::OnLoadConfigButtonPressed()
{
  std::cerr << "FIXME: Load file, load config, ev. report errors, start timer"
      << std::endl;
}

void UNO23ControlWindow::OnTaskStarted(bool execute)
{
  ...
  m_CurrentTask = task;
  UNO23RegistrationExecutionTask *testExec =
      dynamic_cast<UNO23RegistrationExecutionTask *>(m_CurrentTask);
...
void UNO23ControlWindow::OnTaskProgress(bool execute, double progress)
{
  QMutexLocker lock(&m_TaskMutex);
  Task *task = (Task *) sender();
...
void UNO23ControlWindow::FillVolumeInformation()
{
  QString s1 = "";
  QString s2 = "";
  QString s3 = "";
...
void UNO23ControlWindow::OnTaskManagerTaskHasNoInputsDropped(Task* task)
{
  if (task)
  {
    ...
    UNO23RegistrationInitializationTask *testInitTask =
        dynamic_cast<UNO23RegistrationInitializationTask *>(task);
    UNO23RegistrationExecutionTask *testExecTask =
        dynamic_cast<UNO23RegistrationExecutionTask *>(task);
    if (testImporterTask)
    {
      QMessageBox::critical(this, UNO23ControlWindow::tr("Image import failed"),
          UNO23ControlWindow::tr("Obviously at least one image is not available on disk. Cannot resume with registration!"));
    }
    else if (testInitTask)
    {
      QMessageBox::critical(this, UNO23ControlWindow::tr("Image import failed"),
          UNO23ControlWindow::tr("Obviously the import of at least one image failed. Cannot resume with registration!"));
    }
    else if (testExecTask)
    {
      QMessageBox::critical(this, UNO23ControlWindow::tr("Initialization failed"),
          UNO23ControlWindow::tr("Obviously the initialization of the registration (pre-processing, mask-generation ...) failed. Cannot resume with registration!"));
    }
...
void UNO23ControlWindow::OnTaskFinished(bool execute)
{
  QMutexLocker lock(&m_TaskMutex);
  ...
  if (storeTask && m_CastedModel)
  {
    // dynamic_cast succeeds only if type matches (if we've virtual methods):
    UNO23RegistrationInitializationTask *testInitTask =
        dynamic_cast<UNO23RegistrationInitializationTask *>(storeTask);
    if (testInitTask)
    {
      ...
      // correctly updated):
      QList<UNO23RenderViewDialog *> views = m_ToolButtonWindowMap.values();
      for (int k = 0; k < views.size(); k++)
        views[k]->Update(UNO23RenderViewDialog::FORCE_RESET_WINDOW_LEVEL);
          ...
          divergence > m_CastedModel->GetMaxFixedImageAcquisitionDivergingTimeMin())
      {
        ...
        QMessageBox::warning(this,
            UNO23ControlWindow::tr("Diverging reference X-ray acquisition times"),
            UNO23ControlWindow::tr("The reference X-ray images appear to have diverging acquisition times (%1 min) which are out of tolerance (max. %2 min)!\nPlease check whether or not the right X-rays are selected!").arg(s1).arg(s2));
      }
  ...
  if (storeTask && m_CastedModel)
  {
    UNO23SparseRegistrationExecutionTask *testSparseExecTask =
        dynamic_cast<UNO23SparseRegistrationExecutionTask *>(storeTask);
    UNO23RegistrationExecutionTask *testExecTask =
        dynamic_cast<UNO23RegistrationExecutionTask *>(storeTask);
    if (testSparseExecTask) // sparse pre-registration finished
    {
      ...
      if (!m_NoGUI) // GUI mode
      {
        ...
        QString s;
        if (!userCancel)
          s = UNO23ControlWindow::tr("Sparse pre-registration completed in %1 s.");
        else
          s = UNO23ControlWindow::tr("Sparse pre-registration canceled after %1 s.");
        QLocale loc;
    ...
    else if (testExecTask) // auto-registration execution finished
    {
      ...
      if (!m_NoGUI) // GUI mode
      {
        ...
        QString s;
        if (!userCancel)
          s = UNO23ControlWindow::tr("Auto-registration completed in %1 s.");
        else
          s = UNO23ControlWindow::tr("Auto-registration canceled after %1 s.");
        QLocale loc;
        QString fs = loc.toString(regTime, 'f', 2);
        m_StatusWidget->ShowMessage(s.arg(fs));
      }
...
void UNO23ControlWindow::OnCancelStatusButtonPressed()
{
  QMutexLocker lock(&m_TaskMutex);
  if (m_CurrentTask)
...
void UNO23ControlWindow::SetActivationControlsEnabled(bool enable)
{
  ...
  // enough to justify an "OK" because a user may use UNO-2-3-REG for pure
  // manual registration.
...
void UNO23ControlWindow::OnStartButtonPressed()
{
  // -> initiate sparse pre-registration or auto-registration (if configured and
  // requested):
  UNO23SparseRegistrationExecutionTask *preTask = m_CastedModel->
      GenerateSparseExecutionTask();
  if (preTask)
  {
    preTask->OverrideName(UNO23ControlWindow::tr("Sparsely pre-registering ..."));
    preTask->SetRegistrationTimeFormatString(UNO23ControlWindow::tr(" [%1 s]"));
  ...
  else // pre-registration is not requested or not configured -> auto-reg.
  {
    ...
    if (task && m_CastedModel->GetTaskManager())
    {
      task->OverrideName(UNO23ControlWindow::tr("Auto-registering ..."));
      task->SetRegistrationTimeFormatString(UNO23ControlWindow::tr(" [%1 s]"));
    ...
    else
    {
      QMessageBox::critical(
          this,
          UNO23ControlWindow::tr("Automatic registration"),
          UNO23ControlWindow::tr(
...
void UNO23ControlWindow::CreateRenderWindowButtons()
{
  ...
  for (int i = cc - 1; i >= 0; i--) // remove children first
  {
    ...
    if (tb)
    {
      ...
      {
        UNO23RenderViewDialog *vd = m_ToolButtonWindowMap.value(tb);
  ...
  for (std::size_t i = 0; i < vns.size(); i++)
  {
    ...
    if (vns[i].length() > 0)
      ...
      caption = UNO23ControlWindow::tr("View %1").arg(i + 1);
    QToolButton *tb = new QToolButton(ui.RenWinButtonsFrame);
    tb->setText(caption);
    tb->setToolTip(
        UNO23ControlWindow::tr("Activate / deactivate view window (%1)").
        arg(caption));
    ...
    // -> add render window:
    UNO23RenderViewDialog *vd = new UNO23RenderViewDialog(NULL); // not childs
...
void UNO23ControlWindow::OnRenderViewToolButtonToggled(bool checked)
{
  ...
  if (tb)
  {
    if (m_ToolButtonWindowMap.value(tb))
    {
      UNO23RenderViewDialog *d = m_ToolButtonWindowMap.value(tb);
      if (checked)
      {
...
void UNO23ControlWindow::OnRenderViewFinished(int result)
{
  UNO23RenderViewDialog *d = dynamic_cast<UNO23RenderViewDialog*>(sender());
  if (d)
  {
...
void UNO23ControlWindow::AddRenderViewGeometry(int idx, int x, int y, int w, int h, bool visibility)
{
  if (idx < 0 || w <= 0 || h <= 0)
    return;
...
void UNO23ControlWindow::OnGUIUpdateTimerTimeout()
{
  if (m_UpdateCostFunctionFlag)
  {
    ...
    {
      if (xx == 0)
        funcName = UNO23ControlWindow::tr("metric[-]");
      else if (xx == 1)
        funcName = UNO23ControlWindow::tr("rotX[deg]");
      else if (xx == 2)
        funcName = UNO23ControlWindow::tr("rotY[deg]");
      else if (xx == 3)
        funcName = UNO23ControlWindow::tr("rotZ[deg]");
      else if (xx == 4)
        funcName = UNO23ControlWindow::tr("transX[cm]");
      else if (xx == 5)
        funcName = UNO23ControlWindow::tr("transY[cm]");
      else if (xx == 6)
        funcName = UNO23ControlWindow::tr("transZ[cm]");
      else
        funcName = "";
      if (m_StoredCurveVisibility.size() != endcount)
...
void UNO23ControlWindow::OnUndoRedoManagerStackModified()
{
...
void UNO23ControlWindow::OnUndoRedoManagerTaskExecutionRequest(
...
void UNO23ControlWindow::OnUndoMenuTriggered(QAction *action)
{
  if (action)
  {
...
void UNO23ControlWindow::OnRedoMenuTriggered(QAction *action)
{
  if (action)
  {
    QVariant var = action->data();
...
void UNO23ControlWindow::OnUndoToolButtonClicked()
{
  if (m_CastedModel && m_CastedModel->IsReadyForManualRegistration())
  {
    SimpleTransformUndoRedoManager *undoRedoManager = m_CastedModel->GetUndoRedoManager();
...
void UNO23ControlWindow::OnRedoToolButtonClicked()
{
  if (m_CastedModel && m_CastedModel->IsReadyForManualRegistration())
  {
    SimpleTransformUndoRedoManager *undoRedoManager = m_CastedModel->GetUndoRedoManager();
...
void UNO23ControlWindow::OnResetToolButtonClicked()
{
  if (m_CastedModel && m_CastedModel->IsReadyForManualRegistration())
  {
    SimpleTransformUndoRedoManager *undoRedoManager = m_CastedModel->GetUndoRedoManager();
...
void UNO23ControlWindow::OnAboutButtonPressed()
{
  UNO23AboutDialog ad(this);
  ad.exec();
}

void UNO23ControlWindow::OnHelpButtonPressed()
{
  ...
  + "UNO23REG_ShortReference_" + language + ".html";
...
void UNO23ControlWindow::OnHelpLoadStarted()
{
  QApplication::setOverrideCursor(Qt::BusyCursor);
}

void UNO23ControlWindow::OnHelpLoadFinished(bool ok)
{
  QApplication::restoreOverrideCursor();
  m_StatusWidget->ShowMessage("");
}

void UNO23ControlWindow::OnHelpLoadProgress(int progress)
{
  m_StatusWidget->ShowMessage(UNO23ControlWindow::tr("Loading help %1%").arg(progress));
}

void UNO23ControlWindow::SetLayoutConfigurationFile(QString fileName)
{
...
void UNO23ControlWindow::OnLastWindowClosed()
{
  ...
  if (!result)
    return;
  QMap<QToolButton *, UNO23RenderViewDialog *>::const_iterator it;
...
void UNO23ControlWindow::closeEvent(QCloseEvent *event)
{
...
//    {
//      QMessageBox::critical(this, UNO23ControlWindow::tr("Processes running!"),
//        UNO23ControlWindow::tr("There is at least one process running; cannot close the program. Please cancel or finish the process before!"));
//      event->ignore();
//      return;
//    }
    bool doClose = true;
    if (m_CastedModel && m_CastedModel->HaveValidConfiguration()) // only ask here
    {
      ...
      if (m_CastedModel->GetShowWarningOnCancel() &&
          ...
          QMessageBox::question(this, UNO23ControlWindow::tr("Are you sure?"),
          UNO23ControlWindow::tr("Are you sure that you want to 'decline' the actual registration result?"),
...
void UNO23ControlWindow::SetWindowsStayOnTop(bool stayOnTop)
{
...
void UNO23ControlWindow::OnReferenceToolButtonClicked()
{
  if (m_CastedModel->GetReferenceParameters().GetSize() <= 0)
...
void UNO23ControlWindow::OnITFOptimizerToolButtonClicked()
{
  if (m_CastedModel->IsReadyForManualRegistration())
  {
    QString dir = QFileDialog::getExistingDirectory(this,
        UNO23ControlWindow::tr("Select the ITF-optimization storage directory ..."));
    if (dir.length() > 0)
    {
      ...
      if (!ok)
      {
        QMessageBox::critical(this, UNO23ControlWindow::tr("ITF-optimizer configuration error"),
            UNO23ControlWindow::tr("One or more errors occured during generating and storing ITF-optimization configuration!"));
      }
    }
  }
}

void UNO23ControlWindow::OnSaveDRRsToolButtonClicked()
{
  if (m_CastedModel->IsReadyForManualRegistration())
  {
    QString dir = QFileDialog::getExistingDirectory(this,
        UNO23ControlWindow::tr("Select the DRR storage directory ..."));
    if (dir.length() > 0)
    {
      bool ok = false;
      QString pattern = QInputDialog::getText(this,
          UNO23ControlWindow::tr("File name pattern"),
          UNO23ControlWindow::tr("DRR pattern (%d for view index):"),
          QLineEdit::Normal, UNO23ControlWindow::tr("drr%d.mhd"), &ok);
      if (ok && !pattern.isEmpty())
      {
        if (!pattern.contains("%d", Qt::CaseSensitive))
        {
          QMessageBox::critical(this, UNO23ControlWindow::tr("No pattern"),
              UNO23ControlWindow::tr("No %d (view index) was detected in pattern!"));
          return;
        }
        QStringList sl;
        ...
        QString sel = QInputDialog::getItem(this,
            UNO23ControlWindow::tr("Image dimensionality and data type"),
            UNO23ControlWindow::tr("Selection:"), sl, defaultIndex, false, &ok);
        if (ok)
        {
          defaultIndex = -1;
          ...
          if (!ok)
          {
            QMessageBox::critical(this, UNO23ControlWindow::tr("DRR storage error"),
                UNO23ControlWindow::tr("One or more errors occured during generating and storing DRRs!"));
          }
        }
      }
    }
