...
UNO23SparseRegistrationExecutionTask::UNO23SparseRegistrationExecutionTask() :
...
void UNO23SparseRegistrationExecutionTask::OnInternalFilterProgressReceptor(
...
bool UNO23SparseRegistrationExecutionTask::Execute()
{
  ...
  if (!m_CancelRequest)
  {
    ...
    {
      typedef itk::MemberCommand<UNO23SparseRegistrationExecutionTask> ITKCommandType;
      ...
      cmd->SetCallbackFunction(this,
          (MemberPointer)&UNO23SparseRegistrationExecutionTask::OnInternalFilterProgressReceptor);

      // OK, the arguments were checked for correctness, extract them now:
      // CrossCorrelation(<num-it>,<x-max>,<y-max>)
      UNO23Model::ParametersType prevPars = tm->GetCurrentParameters(); // store
      ...
      int index = 0;
      double tx = 0, ty = 0;
      UNO23Model::ParametersType pars;
      ...
      if (maxTRE > 0)
      {
        UNO23Model::TransformType::InputPointType zero;
        zero.Fill(0);
        UNO23Model::TransformType::OutputPointType p = tm->m_Transform->TransformPoint(zero);
...
bool UNO23SparseRegistrationExecutionTask::Unexecute()
{
  this->m_CancelRequest = false;
  emit
  TaskStarted(false);
...
QString UNO23SparseRegistrationExecutionTask::GetName()
{
