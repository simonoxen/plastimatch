...
/**
 * A simple application that updates the UNO-2-3-REG RC (win32 resource) file
 * with the actual UNO-2-3-REG global information. A variety of keys are
...
int main(int argc, char *argv[])
{
  ...
  while(std::getline(ifile, line))
  {
    ...
    {
      s2 = ora::UNO23REGGlobalInformation::ProductVersion;
    ...
    {
      s2 = ora::UNO23REGGlobalInformation::GetCombinedCompanyName();
    ...
    {
      s2 = ora::UNO23REGGlobalInformation::GetCombinedApplicationName();
      std::size_t p = ora::ToLowerCaseF(line).find("value");
      std::string spaces(p, ' ');
    ...
    {
      s2 = ora::UNO23REGGlobalInformation::ApplicationShortName;
      std::size_t p = ora::ToLowerCaseF(line).find("value");
      std::string spaces(p, ' ');
    ...
    {
      s2 = ora::UNO23REGGlobalInformation::Copyright;
      std::size_t p = ora::ToLowerCaseF(line).find("value");
      std::string spaces(p, ' ');
    ...
    {
      s2 = ora::UNO23REGGlobalInformation::GetOriginalApplicationBinaryName();
      std::size_t p = ora::ToLowerCaseF(line).find("value");
      std::string spaces(p, ' ');
    ...
    {
      s2 = ora::UNO23REGGlobalInformation::GetCombinedVersionString();
      std::size_t p = ora::ToLowerCaseF(line).find("value");
      std::string spaces(p, ' ');
  ...
  if (c > 0)
  {
    ...
    ofile << os.str();
    ofile.close();
    std::cout << "*** uno23reg-rc-updater updated " << c << " keys in resource file ***\n";
  }
  else
  {
    std::cout << "*** uno23reg-rc-updater had nothing to change ***\n";
  }
