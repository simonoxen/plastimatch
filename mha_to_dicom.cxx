...
main(int argc, char *argv[])
{
    ...
    if (output_type == PLM_IMG_TYPE_ITK_UCHAR) {
        UCharImageType::Pointer input_image = load_uchar (input_fn, 0);
        itk_image_save_short_dicom (input_image, dicom_dir);
    } else if (output_type == PLM_IMG_TYPE_ITK_SHORT) {
        ShortImageType::Pointer input_image = load_short (input_fn, 0);
        itk_image_save_short_dicom (input_image, dicom_dir);
    } else if (output_type == PLM_IMG_TYPE_ITK_USHORT) {
        UShortImageType::Pointer input_image = load_ushort (input_fn, 0);
        itk_image_save_short_dicom (input_image, dicom_dir);
    } else if (output_type == PLM_IMG_TYPE_ITK_FLOAT) {
        FloatImageType::Pointer input_image = load_float (input_fn, 0);
        itk_image_save_short_dicom (input_image, dicom_dir);
    }
